...
# define RT_MDL_TYPE      RT_MODEL_linear_axis_T
#endif
...
typedef struct {
    ExtU_linear_axis_T ExternalInput;
    ExtY_linear_axis_T ExternalOutput;
    P_linear_axis_T Parameter;
    RT_MODEL_linear_axis_T RTModel;
    B_linear_axis_T blockBuffer;
    DW_linear_axis_T dWork;
    X_linear_axis_T x;
    real_T rListBuffer[18];
    fmi3Float64 time;
...
fmi3Instance fmi3InstantiateModelExchange(
    ...
    if (strcmp(instantiationToken, "{475c1aa8-56d1-a896-a09f-4f99c68a66e9}")) {
        if (loggingOn && logMessage && instanceEnvironment)
    ...
    modelData->rList = (fmi3Float64**)calloc(18, sizeof(fmi3Float64*));
    if (!modelData->rList) {
         if (loggingOn && logMessage && instanceEnvironment)
               logMessage(instanceEnvironment, fmi3Error, "error","fmi3InstantiateModelExchange: Memory allocation error.");
        return NULL;
    }
   modelData->rListSize = (int32_T*)calloc(18, sizeof(int32_T));
    if (!modelData->rListSize) {
         if (loggingOn && logMessage && instanceEnvironment)
               logMessage(instanceEnvironment, fmi3Error, "error","fmi3InstantiateModelExchange: Memory allocation error.");
        return NULL;
    }
   modelData->rListDims = (int32_T**)calloc(18, sizeof(int32_T*));
    ...
    modelData->rOutput = (fmi3Float64*)calloc(3, sizeof(fmi3Float64));
    if (!modelData->rOutput) {
         if (loggingOn && logMessage && instanceEnvironment)
               logMessage(instanceEnvironment, fmi3Error, "error","fmi3InstantiateModelExchange: Memory allocation error.");
        return NULL;
    }
    modelData->derivsBuffer = (fmi3Float64*)calloc(1,sizeof(XDot_linear_axis_T));
    linear_axis_M->derivs = (real_T*)modelData->derivsBuffer;
#ifdef rtmGetSolverZcSignalVector
    modelData->zcBuffer = (fmi3Float64*)calloc(1,sizeof(ZCV_linear_axis_T));
    linear_axis_M->zcSignalVector = (real_T*)modelData->zcBuffer;
#endif
    modelData->rList[0] = (double*)&(linear_axis_U.setpoint);
    modelData->rList[1] = (double*)&(linear_axis_Y.v);
    modelData->rList[2] = (double*)&(linear_axis_Y.s);
    modelData->rList[3] = (double*)&(linear_axis_Y.T);
#ifdef rtmGetTPtr
    modelData->rList[4] = rtmGetTPtr(linear_axis_M);
#else
    modelData->rList[4] = &(modelData->time);
#endif
    modelData->rList[5] = (double*)&(((double*)linear_axis_M->contStates)[0]);
    modelData->rList[6] = (double*)&(((double*)linear_axis_M->contStates)[1]);
    modelData->rList[7] = (double*)&(((double*)linear_axis_M->contStates)[2]);
    modelData->rList[8] = (double*)&(((double*)linear_axis_M->contStates)[3]);
    modelData->rList[9] = (double*)&(((double*)linear_axis_M->derivs)[0]);
    modelData->rList[10] = (double*)&(((double*)linear_axis_M->derivs)[1]);
    modelData->rList[11] = (double*)&(((double*)linear_axis_M->derivs)[2]);
    modelData->rList[12] = (double*)&(((double*)linear_axis_M->derivs)[3]);
#ifdef rtmGetSolverZcSignalVector
    modelData->rList[13] = (double*)&(((double*)linear_axis_M->zcSignalVector)[0]);
#endif
#ifdef rtmGetSolverZcSignalVector
    modelData->rList[14] = (double*)&(((double*)linear_axis_M->zcSignalVector)[1]);
#endif
#ifdef rtmGetSolverZcSignalVector
    modelData->rList[15] = (double*)&(((double*)linear_axis_M->zcSignalVector)[2]);
#endif
#ifdef rtmGetSolverZcSignalVector
    modelData->rList[16] = (double*)&(((double*)linear_axis_M->zcSignalVector)[3]);
#endif
#ifdef rtmGetSolverZcSignalVector
    modelData->rList[17] = (double*)&(((double*)linear_axis_M->zcSignalVector)[4]);
#endif
...
fmi3Status fmi3GetFMUState (fmi3Instance c, fmi3FMUState* FMUState) {
    ...
    memcpy(&fmuInternalState->ExternalInput, &linear_axis_U, sizeof(ExtU_linear_axis_T));
    memcpy(&fmuInternalState->ExternalOutput, &linear_axis_Y, sizeof(ExtY_linear_axis_T));
    memcpy(&fmuInternalState->Parameter, &linear_axis_P, sizeof(P_linear_axis_T));
    memcpy(&fmuInternalState->RTModel, modelData->S, sizeof(RT_MODEL_linear_axis_T));
    memcpy(&fmuInternalState->blockBuffer, &linear_axis_B, sizeof(B_linear_axis_T));
    memcpy(&fmuInternalState->dWork, &linear_axis_DW, sizeof(DW_linear_axis_T));
    memcpy(&fmuInternalState->x, &linear_axis_X, sizeof(X_linear_axis_T));
...
fmi3Status fmi3SetFMUState (fmi3Instance c, fmi3FMUState FMUState) {
    ...
    memcpy(&linear_axis_U, &fmuInternalState->ExternalInput, sizeof(ExtU_linear_axis_T));
    memcpy(&linear_axis_Y, &fmuInternalState->ExternalOutput, sizeof(ExtY_linear_axis_T));
    memcpy(&linear_axis_P, &fmuInternalState->Parameter, sizeof(P_linear_axis_T));
    ...
    memcpy(&linear_axis_B, &fmuInternalState->blockBuffer, sizeof(B_linear_axis_T));
    memcpy(&linear_axis_DW, &fmuInternalState->dWork, sizeof(DW_linear_axis_T));
    memcpy(&linear_axis_X, &fmuInternalState->x, sizeof(X_linear_axis_T));
    for(size_t i = 0; i < 18; i++) {
...
fmi3Status fmi3GetContinuousStates(fmi3Instance c, fmi3Float64  x[], size_t nx) {
    fmi3ValueReference vr[4] = {5,6,7,8};
    (void)c;
    size_t nvr = 4;
...
fmi3Status fmi3GetContinuousStateDerivatives(fmi3Instance c, fmi3Float64 derivatives[], size_t nx) {
    fmi3ValueReference vr[4] = {9,10,11,12};
    (void)c;
    FMUModelData *modelData = (FMUModelData *)c;
    _instance = modelData;
    linear_axis_derivatives();
    size_t nvr = 4;
...
fmi3Status fmi3GetEventIndicators(fmi3Instance c, fmi3Float64  eventIndicators[], size_t ni) {
    (void)c;
#ifdef rtmGetSolverZcSignalVector
    linear_axis_zeroCrossings();
    eventIndicators[0] = ((double*)linear_axis_M->zcSignalVector)[0];
    eventIndicators[1] = ((double*)linear_axis_M->zcSignalVector)[1];
    eventIndicators[2] = ((double*)linear_axis_M->zcSignalVector)[2];
    eventIndicators[3] = ((double*)linear_axis_M->zcSignalVector)[3];
    eventIndicators[4] = ((double*)linear_axis_M->zcSignalVector)[4];
#endif
    return fmi3OK;
}
